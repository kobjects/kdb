...
public class HttpResultSet implements DbResultSet {
	...
	HttpResultSet(HttpTableSE table, int[] fields, Vector selection) {
		this.table = table;
		...
		try {
			...
			if (connection.getResponseCode() != 200)
				throw new IOException(
					"bad response code: " + connection.getResponseCode());
		}
		catch (Exception e) {
			e.printStackTrace();
